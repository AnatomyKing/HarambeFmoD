...
public final class HarambeCore {
    ...
    public HarambeCore(IEventBus modBus, ModContainer container) {
        ModEntities.register(modBus);          // entities + attributes
        ModItems.register(modBus);             // items (helpers used)
        ModDataComponents.DATA_COMPONENTS.register(modBus);
        ModBlocks.register(modBus);            // blocks + auto block items (helpers used)
        ModBlockEntities.register(modBus);     // block entities
    ...
    public static final class ClientEvents {
        ...
        public static void registerBlockColors(RegisterColorHandlersEvent.Block e) {
            // Pearl Fire tint from its BE (tintindex 0)
            e.register((state, level, pos, tintIndex) -> {
                if (tintIndex == 0 && level != null && pos != null) {
                    BlockEntity be = level.getBlockEntity(pos);
                    if (be instanceof net.anatomyworld.harambefmod.block.entity.PearlFireBlockEntity pf) {
                        return pf.getColor();
                    }
                }
                return 0xFFFFFFFF;
            ...
            // Banana Portal tint (cache → BE → default), tintindex 0
            e.register((state, level, pos, tintIndex) -> {
                ...
                BlockEntity be = level.getBlockEntity(pos);
                if (be instanceof net.anatomyworld.harambefmod.block.entity.BananaPortalBlockEntity p) return p.getColor();
                return 0xFFFFFFFF;
