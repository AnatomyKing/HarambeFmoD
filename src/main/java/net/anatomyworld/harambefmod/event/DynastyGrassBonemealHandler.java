...
public final class DynastyGrassBonemealHandler {
    private DynastyGrassBonemealHandler() {}
    ...
    public static void onBonemeal(BonemealEvent event) {
        ...
        for (Direction d : Direction.Plane.HORIZONTAL) {
            if (level.getBlockState(pos.relative(d)).is(ModBlocks.DYNASTY_GRASS_BLOCK.get())) {

                if (level instanceof ServerLevel sl) {
                    // 1) convert the block
                    sl.setBlock(pos, ModBlocks.DYNASTY_GRASS_BLOCK.get().defaultBlockState(), 3);
