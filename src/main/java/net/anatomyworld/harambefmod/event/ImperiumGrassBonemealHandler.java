...
public final class ImperiumGrassBonemealHandler {
    private ImperiumGrassBonemealHandler() {}
    ...
    public static void onBonemeal(BonemealEvent event) {
        ...
        for (Direction d : Direction.Plane.HORIZONTAL) {
            if (level.getBlockState(pos.relative(d)).is(ModBlocks.IMPERIUM_GRASS_BLOCK.get())) {

                if (level instanceof ServerLevel sl) {
                    // 1) convert the block
                    sl.setBlock(pos, ModBlocks.IMPERIUM_GRASS_BLOCK.get().defaultBlockState(), 3);
